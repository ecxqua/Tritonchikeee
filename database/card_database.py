...
Таблицы:
    ...
    2. cards — карточки особей (FK project_id)
    3. photos — фотографии особей (FK card_id)
...
def init_database():
    ...
    # -------------------------------------------------------------------------
    # ТАБЛИЦА 2: cards (карточки особей) — FK project_id
    # -------------------------------------------------------------------------
    cursor.execute('''
    ...
    # --- cards ---
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_cards_project ON cards(project_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_cards_template ON cards(template_type)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_cards_species ON cards(species)')
    
    # --- photos ---
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_photos_embedding ON photos(embedding_index)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_photos_individual ON photos(card_id)')
