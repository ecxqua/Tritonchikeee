...
async def handle_photo(message: types.Message, bot: Bot):
    ...
    try:
        ...
        filename = "image.jpg"
        filepath = os.path.join(bot.config["io"]["input_folder"], filename)
...
async def send_results(message: types.Message, bot: Bot):
    ...
    try:
        ...
        result_files = [
            os.path.join(bot.config["io"]["output_folder"], f"top{i}.jpg")
            for i in range(1, bot.config["view"]["size_answer"] + 1)
        ]
        ...
        res_file_path = os.path.join(bot.config["io"]["output_folder"], "res.txt")
        if os.path.exists(res_file_path):
            ...
            media_group = []
            for i, filepath in enumerate(existing_files[:bot.config["view"]["size_answer"]]):
